...
| BASE_GRADLE_FLAGS   	                              | &check;      | -s --no-daemon -PnoMavenLocal --console=plain  (-PsafeTest)	           | Default Gradle flags that will be appended to all Gradle commands (Will include -PsafeTest when SAFE_TEST is set to "true")                                                                                  |
...
| GRADLE_TEST_FLAGS                 	                | &check;      | -s --no-daemon -PnoMavenLocal --console=plain 	                        | Gradle flags that will be appended when running the Test Gradle task(s). 	                                                                                                                                   |
...
| EXTRA_GRADLE_FLAGS                 	 |                                                                     | Any extra gradle flags                                                                                                                                                                                        |
| STANDARD_GRADLE_FLAGS   	            | -s --no-daemon -PnoMavenLocal --console=plain  (-PsafeTest)         | Default Gradle flags that will be appended to all Gradle commands 	                                                                                                                                           |
...
| STANDARD_GRADLE_FLAGS | -s --no-daemon -PnoMavenLocal --console=plain $TASK_ARGUMENTS | Default Gradle flags that will be appended to all Gradle commands                                                                                        |
| TASK_ARGUMENTS        |                                                               | Additional command line arguments and gradle tasks for this build. ex: \"-Pforce -x updateReleaseVersion\" These tasks will run on every job downstream. |
...
| STANDARD_GRADLE_FLAGS                         | '-s --no-daemon -PnoMavenLocal --console=plain'                                                     | Default Gradle flags that will be appended to all Gradle commands                                                                                                                           |
