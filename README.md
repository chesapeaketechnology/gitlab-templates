...
| DEV_OR_RELEASE_REGEX                                   | &check;      | `^develop$`\|`^v3-develop$`\|`^v2-develop$`\|`^[0-9]+\.[0-9]+$`\|`^release\/.+$`   | Branch(es) jobs will be run from when new commits are made. For example, if it's desired to run jobs from from `v2-develop` and `v3-develop` branches, this variable can be set to `'^v3-develop\ |$^v2-develop$'`
